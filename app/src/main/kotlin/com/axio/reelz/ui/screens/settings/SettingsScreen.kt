...
fun StorageUsageScreen(nav: NavController) {
    ...
    //
    // totalBytes  — actual physical capacity of the device storage partition.
    //               We read from the internal-storage StatFs because Android
    //               reports the real total there (external/sdcard can report
    //               a smaller logical partition on some OEMs).
    //
    // freeBytes   — bytes actually available to apps right now.
    //
    // reelzBytes  — sum of every file inside Reelz's private app-external
    //               downloads folder (getExternalFilesDir). This correctly
    //               counts HLS segments, MP4 files, and subtitle tracks.
    //               Falls back to internal storage folder if external is null.
    //
    // The "Used by System & Apps" row has been removed — it's not relevant
    // to the user in this context and the number was misleading.

    val totalBytes: Long = remember {
        // Use the internal /data partition for the authoritative total —
        // this matches what Android Settings shows.
        android.os.StatFs(context.filesDir.absolutePath).totalBytes
    }
    val freeBytes: Long = remember {
    ...
    val reelzBytes: Long = remember {
        // Sum every file under the app's external "reelz_downloads" sub-folder.
        // getExternalFilesDir(null) → e.g. /sdcard/Android/data/com.axio.reelz/files
        // The engine stores downloads one level deeper at reelz_downloads/<id>/
        val extBase = context.getExternalFilesDir(null)
        val dirs = buildList {
            if (extBase != null) add(java.io.File(extBase, "reelz_downloads"))
            // Also check internal fallback path the engine uses when external is null
            add(java.io.File(context.filesDir, "reelz_downloads"))
        }
        dirs.sumOf { dir ->
            if (dir.exists()) {
    ...
    Column(
        ...
        Column(
            ...
            // "Used by System & Apps" removed — irrelevant to Reelz context.
            listOf(
