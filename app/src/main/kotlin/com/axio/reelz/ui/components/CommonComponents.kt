...
// ─────────────────────────────────────────────────────────────────────────────
// FeedbackIconButton — placed at the five feedback entry points.
//
// Surfaces:
//   SETTINGS  → general, no request_id or tmdb_id
//   DETAIL    → pass tmdb_id (catalog, no request_id)
//   DOWNLOAD  → pass request_id from active download card
//   PLAYER    → pass request_id from engine response
//   SHORTS    → opened via ShortsDrawer, not this button directly
//
// Usage:
//   FeedbackIconButton(
//       source    = FeedbackSource.PLAYER,
//       requestId = streamRequestId,
//       onOpen    = { nav.navigate(Route.Feedback.build("player", requestId = streamRequestId)) }
//   )
// ─────────────────────────────────────────────────────────────────────────────

val FeedbackFlagIcon: ImageVector get() =
    ImageVector.Builder("FeedbackFlag", 24.dp, 24.dp, 24f, 24f).apply {
        addPath(
            pathData = PathData {
                moveTo(4f, 15f)
                ...
                lineTo(20f, 5f)
                curveTo(20f, 5f, 19f, 6f, 16f, 6f)
                curveTo(13f, 6f, 11f, 4f, 8f, 4f)
                curveTo(5f, 4f, 4f, 5f, 4f, 5f)
                lineTo(4f, 21f)
            },
...
fun FeedbackIconButton(
    ...
    if (withBg) {
        Box(
            ...
            Icon(FeedbackFlagIcon, contentDescription = "Report / Feedback", tint = tint, modifier = Modifier.size(iconSize))
        }
    } else {
