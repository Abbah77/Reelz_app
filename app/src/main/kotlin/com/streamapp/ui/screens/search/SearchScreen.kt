    ...
    init {
        viewModelScope.launch {
            _query.debounce(400).collectLatest { q ->
                ...
                } catch (e: Exception) { SearchState.Error(e.message ?: "Search failed") }
            }
        }
    }
    ...
    Column(
        ...
        Row(
            ...
            OutlinedTextField(
                ...
                leadingIcon = { Icon(Icons.Filled.Search, null, tint = White60) },
                trailingIcon = {
                    if (query.isNotEmpty()) {
                        IconButton(onClick = viewModel::clear) {
                            Icon(Icons.Filled.Close, null, tint = White60)
                        }
                    }
                }
            )
        ...
        when (val s = state) {
            is SearchState.Idle -> SearchHint()
            is SearchState.Loading -> Box(Modifier.fillMaxSize()) {
                CircularProgressIndicator(Modifier.align(Alignment.Center), color = Primary)
            }
            is SearchState.Empty -> Box(Modifier.fillMaxSize()) {
                Column(Modifier.align(Alignment.Center), horizontalAlignment = Alignment.CenterHorizontally) {
                    Text("🔍", fontSize = 40.sp)
                    Spacer(Modifier.height(8.dp))
                    Text("No results for \"$query\"", color = White60, style = MaterialTheme.typography.bodyMedium)
                }
            }
            ...
            is SearchState.Error -> Box(Modifier.fillMaxSize()) {
...
private fun SearchHint() {
    ...
    Column(Modifier.fillMaxSize().padding(20.dp)) {
        Text("Trending Searches", style = MaterialTheme.typography.headlineSmall, color = White)
                ...
                row.forEach { tag ->
                    Surface(
                        ...
                        Row(
                            ...
                            Icon(Icons.Filled.TrendingUp, null, tint = PrimaryLight, modifier = Modifier.size(14.dp))
