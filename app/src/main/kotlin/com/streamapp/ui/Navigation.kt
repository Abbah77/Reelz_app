...
sealed class Screen(val route: String) {
    ...
    object Player : Screen("player/{url}/{title}/{episode}") {
        fun create(url: String, title: String, episode: String = "") =
            "player/${URLEncoder.encode(url,"UTF-8")}/${URLEncoder.encode(title,"UTF-8")}/${URLEncoder.encode(episode,"UTF-8")}"
    }
}

data class BottomNavItem(val screen: Screen, val label: String, val icon: ImageVector, val iconSelected: ImageVector)
                ...
                NavigationBar(
                    ...
                    bottomNavItems.forEach { item ->
                        ...
                        NavigationBarItem(
                            ...
                            icon = {
                                Icon(if (selected) item.iconSelected else item.icon, item.label)
                            },
            ...
            composable(
                ...
                DetailScreen(
                    ...
                    onPlay = { url ->
                        ...
                        navController.navigate(Screen.Player.create(url, title))
                    },
                    onItemClick = { item ->
                        navController.navigate(Screen.Detail.create(item.id, item.mediaType))
                    }
